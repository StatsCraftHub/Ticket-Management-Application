...
# Define authentication credentials (username and password)
...
authentication <- function(username, password) {
  user_info <- credentials[credentials$user == username, ]
  if (nrow(user_info) == 0) {
...
ui_auth <- secure_app(
  shinydashboard::dashboardPage(
    shinydashboard::dashboardHeader(title = "Pharma Ticket Management"),
    shinydashboard::dashboardSidebar(
      shinydashboard::sidebarMenu(
        shinydashboard::menuItem("Create Ticket", tabName = "create"),
        shinydashboard::menuItem("View Tickets", tabName = "view"),
        shinydashboard::menuItem("Last 3 Months Status", tabName = "status_chart")
      )
    ),
...
server <- function(input, output, session) {
  ...
  output$ticketTable <- renderDT({
    ...
    datatable(tickets, options = list(pageLength = 10), class = "ticket-table")
  })
